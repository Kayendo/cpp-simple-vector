...
public:
	...
	void Resize(size_t new_size) {                                                             
	    ...
	    if (new_size < size_) {
	        for (auto it = std::make_move_iterator(&items_[new_size]); it != std::make_move_iterator(&items_[size_]); ++it) {
	            *(it) = Type{};
	        }
	    }
	...
	void PushBack(Type&& item) {
        if (size_ < capacity_) {
            ...
            auto new_capacity = capacity_ ? 2 * capacity_ : 1; //защита, если capacity_=0
            ArrayPtr<Type> arr_ptr(new_capacity);
            std::copy(std::make_move_iterator(&items_[0]), std::make_move_iterator(&items_[size_]), &arr_ptr[0]);
            arr_ptr[size_] = std::move(item);
            std::swap(items_, arr_ptr);
            capacity_ = new_capacity;
        }
        ++size_;
    }
